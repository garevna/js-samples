...
function DataLoadingWorkerConstructor () {
	try {
		...
		var x = new Modals ( {
			type: 'alert',
			text: "К сожалению, в Вашем браузере полная функциональность невозможна"
		} );
		return false
	}
	...
	worker.onmessage = function ( mess ) {
		if ( mess.data ) {
			if ( this.callback ) 
				this.callback ( mess.data, this.params )
			else 
				var x = new Modals ( {
					type: 'alert',
					text: "data worker callback undefined"
				} )
		} else 
			var x = new Modals ( {
				type: 'alert',
				text: "Отсутствуют необходимые библиотеки: "  + this.sourceURL
				} )
	}
	return worker
}
...
SceneForStudyProcessConstructor.prototype.createWorkPanel = function () {
	...
	panel.addFolderStateButton = function ( folderNum ) {
		...
		_folderStateButton.onclick = function ( event ) {
			...
			// ----------------------------- Animation ---------------------------------------------------
			if ( _state == 1 )
				TweenLite.to ( _content, 1, { height:"0", opacity:"0", display:"none" } )
