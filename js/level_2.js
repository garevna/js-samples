...
function SceneForStudyProcessConstructor () {
	...
	this.__main__.style = "position:fixed;top:0;left:0;bottom:0;right:0;backgroundColor:#000"
	...
	console.info ( 'Источник данных: ' + jsonUrl )
...
function DataLoadingWorkerConstructor () {
	...
	worker.onmessage = function ( mess ) {
		console.info ("data worker sourceURL: " + this.sourceURL)
...
SceneForStudyProcessConstructor.prototype.loadData = function ( sourceURL, callback, params ) {
	...
	console.log ( 'SceneForStudyProcessConstructor sourceURL:', sourceURL )
	console.log ( 'SceneForStudyProcessConstructor callback:', callback )
	console.log ( 'SceneForStudyProcessConstructor params:', params )
...
SceneForStudyProcessConstructor.prototype.mainCallback = function ( response ) {
	...
	for ( var j = 0; j < meta.length; j++ ) {
		if ( meta.item(j).name == "description" )
			meta.item(j).content =  $data.meta_descr
		if ( meta.item(j).name == "keywords" )
			meta.item(j).content =  $data.meta_keywords
	}
	...
	for ( i = 0; i < $data.src.length; i++ ) {
		...
		this.parentObject.processRecord ( $data.src [i] )
	}
}
...
SceneForStudyProcessConstructor.prototype.generateMethod = function ( elementData ) {
	...
	// =============================================================================================================
		...
		func [ 'google_disk' ] = function () {
			var ref = elementData.ref;
			var $url = "https://drive.google.com/folderview?id=" +
											ref + '&usp=sharing'
			window.open ( $url, '_blank' )
		}
		func [ 'div' ] = function () {
			var ref = elementData.ref
			var $url = ( ref.split ( "&lt;" ).join ( "<" ) )
											.split ( "&gt;" ).join ( ">" )
		...
		func [ 'swf' ] = function () {
			if ( !elem_src.parent_node.firstChild ) {
				...
				if ( elementData._background )
						__parent__.style.backgroundColor = elementData._background
				this.browserResize ()
			}
		}
		func [ 'html' ] = function () { }
...
SceneForStudyProcessConstructor.prototype.createWorkPanel = function () {
	...
	panel.addFolderFormButton = function ( folderNum ) {
			...
			_folderFormButton.onclick = function ( event ) {
		        ...
		        this.style.backgroundImage = 'url(' +
										this.folderForms ()[ _folder.form ].$img + ')'
		        this.setAttribute ( 'title',
										this.folderForms () [ _folder.form ].titl )
				...
				for ( var j = 0; j < items.length; j++ ) {
					...
					items [j].className = ( _folder.form == 1 ) ?
												'work-panel-elem-short' : 'work-panel-elem'
				}
			}
	}
	// -------------------------------------------------------------------------------- createDomElements
	...
	panel.createPanelContent = function () {
		...
		this.appendChild ( this.__panelContent__ )
		return this.__panelContent__
	}
	// ------------------------------------------------------------------------------- createFolderElement
   panel.createFolderElement = function ( folderNum ) {
	   if ( !this.__panelContent__ ) {
			 console.log ( 'createFolderElement: Unable to add folder!' )
			 return false
		 }
   ...
   panel.addFolderContent = function ( folderNum ) {
	   ...
	   this.__folders__ [ folderNum ].__content__ =
		 											document.createElement ( 'div' )
	   this.__folders__ [ folderNum ].appendChild (
			 									this.__folders__ [ folderNum ].__content__ )
	   this.__folders__ [ folderNum ].__content__.elem = []
		 console.log ('Line 473: ', this.__folders__ [ folderNum ].__content__)
	   var $form = this.__folders__ [ folderNum ].form ?
		 												this.__folders__ [ folderNum ].form : 0
	...
	panel.folderForms = ( function () {
		var folder_forms = [
		    {
					$img: '/buttons/menu_buttons.png',
					titl: 'Краткая форма (в виде кнопок)'
				},
	      {
					$img: '/buttons/menu-list.png',
					titl: 'Развернутая форма (список)'
				}
			]
		return function (j) {
	...
	panel.getButtonType = function ( elem ) {
		...
		var bt = button_type [ elem.type ] ? button_type [ elem.type ] :
										( elem.button_type ? elem.button_type : 'show' )
		if ( bt == 'show' ) { console.log( 'Не определен тип элемента меню:' +
																												elem.item_name ) }
		return "button_" + bt
	}
    return panel
...
function ElemTooltip ( elem, tooltip_text ) {
	...
	elem.onmouseover = function ( event ) {
		...
		__tooltip__.style = `position: fixed;
													min-width: 150px;
