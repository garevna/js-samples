...
const getRandomRadius = () => Math.max(30, Math.round(Math.random() * 200))
...
elems.iterator = (async function * () {
  ...
  const promise = (size, color) => new Promise(resolve => setTimeout(() => resolve(addElem(size, color)), Math.round(5000 - Math.random() * 4000)))
  let len = this.length
  while (len --> 0) {
    let elem = await promise(this[len].size, this[len].backColor)
    customElements
      .whenDefined('circle-element')
      .then(() => elem.setStyle())
    yield elem
  }
}).call(elems)
...
async function iterateElements () {
  for (var item of elems) await elems.iterator.next()
}
...
class CircleElement extends HTMLElement {
  constructor() {
    ...
    this.shadowStyle = this.shadow.appendChild(document.createElement('style'))
