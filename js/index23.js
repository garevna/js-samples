...
const showAnswer = message => writeHTML ( `<p style="color:yellow">${message}</p>` )

const showError = message => writeHTML ( `<p style="color:red">${message}</p>` )

const getPromise = ( operation, result, fail ) =>
    new Promise (
        ( resolve, reject ) => {
            showOperation ( operation )
            setTimeout (
               () => genBoolean () ? resolve ( result ) : reject ( fail ),
...
async function voyage () {
    ...
    result ? await messages.iterator.next() : null
}
